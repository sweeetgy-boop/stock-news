...
`credit-kiwoom` 은 앱키가 없거나 대상이 0종목이면 exit 4 로 즉시 끝나고,
`nightly.py` 가 이를 '기대된 스킵'으로 셉니다. 같은 exit 4 라도 전 종목
조회가 실패한 경우(예: 8050 지정단말기 인증 실패)는 **실패**로 세고, 완료
알림에 `credit-kiwoom 실패: 8050 × N건` 줄이 붙습니다.

완주 마커의 `reason` 은 셋입니다. `완주` · `부분 완료`(실패 단계는 없지만
`update` 적재 0건) · `부분 실패`(실패 단계 있음). 완료 알림 첫 줄도
`부분 완료` 로 나갑니다.
...
마십시오(1장·6장). 앱키가 없거나 대상이 0종목이면 exit 4 로 즉시 끝나고
파이프라인은 '기대된 스킵'으로 계속 갑니다. 전 종목 조회 실패(8050 등)는
스킵이 아니라 실패로 집계됩니다.
