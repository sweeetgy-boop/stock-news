...
### DART 조회 시장 — 코스닥 확장 (기본 OFF)

증자·감사의견 공시 스캔(`list.json`)은 기본으로 **코스피(corp_cls=Y)만** 본다.
유니버스에는 코스닥 1,724종목이 이미 들어 있고 자본잠식(corp_code 기준)도
코스닥까지 계산되지만, 증자·감사의견 플래그만 코스닥이 0이었다.
...
★ `DART_LIST_MAX_PAGES["Y"]=20` 은 실측 필요량(105)의 1/5 라 최근 약 11일치만
읽는다. 코스닥 확장 중 발견한 기존 결함이다. 코스닥을 켤 때 Y 도 160 으로
함께 올리는 것을 권한다 (호출 +85회, +2분). 상한에 닿으면 flags 로그에
`페이지 상한 도달` 경고가 남는다.
